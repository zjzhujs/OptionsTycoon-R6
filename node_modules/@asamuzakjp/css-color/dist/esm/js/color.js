...
var transformRgbToLinearRgb = (rgb, skip = false) => {
	...
	if (r > COND_POW) r = Math.pow((r + LINEAR_OFFSET) / 1.055, POW_LINEAR);
	else r /= LINEAR_COEF;
	if (g > COND_POW) g = Math.pow((g + LINEAR_OFFSET) / 1.055, POW_LINEAR);
	else g /= LINEAR_COEF;
	if (b > COND_POW) b = Math.pow((b + LINEAR_OFFSET) / 1.055, POW_LINEAR);
	else b /= LINEAR_COEF;
	return [
		r,
		g,
...
var transformLinearRgbToRgb = (rgb, round = false) => {
	...
	if (r > COND_POW) r = Math.pow(r, 1 / POW_LINEAR) * 1.055 - LINEAR_OFFSET;
	else r *= LINEAR_COEF;
	r *= MAX_RGB;
	if (g > COND_POW) g = Math.pow(g, 1 / POW_LINEAR) * 1.055 - LINEAR_OFFSET;
	else g *= LINEAR_COEF;
	g *= MAX_RGB;
	if (b > COND_POW) b = Math.pow(b, 1 / POW_LINEAR) * 1.055 - LINEAR_OFFSET;
	else b *= LINEAR_COEF;
	b *= MAX_RGB;
	return [
...
var parseColorFunc = (value, opt = {}) => {
	...
	if (cs === "srgb-linear") {
		...
		const rgb = [
			...
			let cl;
			if (c > 1 / 32) cl = Math.pow(c, POW_PROPHOTO);
			else cl = c / HEX;
			return cl;
		});
...
var parseColorValue = (value, opt = {}) => {
	...
	if (REG_CURRENT.test(value)) {
		...
		if (Object.hasOwn(NAMED_COLORS, value)) {
			...
			case VAL_COMP:
				if (nullable && value !== "transparent") return null;
				return [
					"rgb",
					0,
					0,
...
var convertColorToXyz = (value, opt = {}) => {
	...
	if (format === "mixValue") {
		...
		const [cs] = val.match(/[^\s,/]+/g);
		if (d50) {
			if (cs === "xyz-d50") [, x, y, z, alpha] = resolveColorFunc(value, { format: VAL_COMP });
...
* @returns resolved color - [cs, v1, v2, v3, alpha], '(empty)', null
*/
var resolveColorMix = (value, opt = {}, resolver = () => null) => {
	if (isString(value)) value = value.toLowerCase().trim();
	else throw new TypeError(`${value} is not a string.`);
	const { format = "", nullable = false } = opt;
	const cacheKey = createCacheKey({
	...
	if (!REG_MIX.test(value)) {
		if (value.startsWith("color-mix(") && REG_MIX_NEST.test(value)) {
			const items = value.match(REG_MIX_NEST);
			for (const item of items) if (item) {
				let val = resolveColorMix(item, { format: format === "specifiedValue" ? format : VAL_COMP }, resolver);
			...
			const [csPart = "", partA = "", partB = ""] = splitValue(colorParts, { delimiter: "," });
			const [colorPartA = "", pctPartA = ""] = splitValue(partA);
			const [colorPartB = "", pctPartB = ""] = splitValue(partB);
			const specifiedColorA = resolver(colorPartA, { format: VAL_SPEC });
			const specifiedColorB = resolver(colorPartB, { format: VAL_SPEC });
	...
	if (/^srgb(?:-linear)?$/.test(colorSpace)) {
		...
		if (rgbA === null || rgbB === null) return cacheInvalidColorValue(cacheKey, format, nullable);
		...
		if (xyzA === null || xyzB === null) return cacheInvalidColorValue(cacheKey, format, nullable);
		...
		if (hslA === null || hslB === null) return cacheInvalidColorValue(cacheKey, format, nullable);
		...
		if (lchA === null || lchB === null) return cacheInvalidColorValue(cacheKey, format, nullable);
		...
		if (labA === null || labB === null) return cacheInvalidColorValue(cacheKey, format, nullable);

...
class LightsSticks {
  ...
  init() {
    ...
    if (Array.isArray(options.colors.sticks)) {
      colorArray = (options.colors.sticks as number[]).map(c => new THREE.Color(c));
    } else {
...
class App {
  ...
  speedUp!: number;
  timeOffset!: number;
  hasValidSize: boolean = false;
  rafId: number | null = null;

  constructor(container: HTMLElement, options: HyperspeedOptions) {
    this.options = options;
    if (!this.options.distortion || typeof this.options.distortion === 'string') {
      const distName = (this.options.distortion as string) || 'turbulentDistortion';
      this.options.distortion = distortions[distName] || {
    ...
    const initW = Math.max(1, container.offsetWidth || window.innerWidth);
    const initH = Math.max(1, container.offsetHeight || window.innerHeight);
...
export const Hyperspeed: FC<HyperspeedProps> = ({
  effectOptions = hyperspeedPresets.optionsTycoon,
  className = '',
  style
}) => {
  ...
  useEffect(() => {
    ...
    const app = new App(hyperspeedRef.current, {
      ...hyperspeedPresets.optionsTycoon,
      ...effectOptions

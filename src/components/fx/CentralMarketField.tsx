...
const FIELD_PALETTES: Record<FieldTheme, FieldPalette> = {
  ...
  amber: {
    ...
    secondary: [2.64, 1.30, 0.30],
    hot: [3.20, 2.62, 1.34],
    tissuePrimary: [0.28, 1.58, 1.82],
    tissueSecondary: [1.72, 0.88, 0.22],
    spine: [3.70, 1.88, 0.44],
    bloom: 1.76,
  },
  calm: {
    primary: [0.46, 1.56, 1.86],
    secondary: [0.62, 1.14, 1.72],
    hot: [1.72, 2.08, 2.18],
    tissuePrimary: [0.28, 1.12, 1.34],
    tissueSecondary: [0.34, 0.76, 1.28],
    spine: [1.56, 1.98, 2.28],
    bloom: 1.34,
  },
};
...
function setupMarketField(
  ...
  const applyTheme = (): void => {
    ...
    const pointGain = theme === 'amber' ? 6.00 : theme === 'calm' ? 6.50 : 1.30;
    const pointSizeGain = theme === 'amber' ? 1.85 : theme === 'calm' ? 1.70 : 1.10;
    ...
    edgeUniforms.uAlphaGain.value = theme === 'amber' ? 8.50 : theme === 'calm' ? 10.00 : 1.32;

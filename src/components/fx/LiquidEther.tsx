...
const defaultColors = ['#040a14', '#0d223a', '#00f0ff', '#d4af37', '#ffd700'];
...
export function LiquidEther({
  ...
  // 减少动效：这个组件的 WebGL 类在 effect 里只构造一次，闭包会把数值锁死，
  // 所以用 ref 传当前值，让类里的 update() 每帧读到最新的。
  ...
  useEffect(() => {
    ...
    const baseCol = new THREE.Color((colors && colors[0]) || '#030712');
    const bgVec4 = new THREE.Vector4(baseCol.r, baseCol.g, baseCol.b, 0.65);
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      start() {
        // 减少动效的闸放在 start() 里，而不是放在调用点：
        // 这个组件有三条独立的启动路径——挂载、IntersectionObserver（进入视口）、
        // visibilitychange（切回标签页）。我先在挂载处 pause()，
        // 结果 IO 回调在下一帧异步触发又把它 start() 回去了。
        // 守在唯一的入口上，三条路径就都堵住了。
        if (motionScaleRef.current === 0) return;
        if (this.running) return;
        this.running = true;
        this._loop();
      }
    ...
    // 减少动效：直接停整个 WebGL 循环，不要试图把时间步长乘 0。
    // 我先试过改 Common.delta，无效——流体求解器用的是固定的 options.dt
    // （prop 默认 0.014），根本不读 Common.delta；而且 autoDriver 用
    // performance.now() 自己造鼠标力，即使时间步长归零它也照样往场里灌力。
    // pause() 是组件自带的总闸（离屏时走的就是这条路，已经验证过），
    // 停在最后一帧上，画面保留颜色与构图，只是不再流动。
    if (motionScaleRef.current === 0) webgl.pause();
  ...
  // 玩家在游戏里勾选/取消「减少动效」时立刻生效，而不是等下次重挂。
  // 恢复时要连 isVisibleRef 一起判断，否则会把一个已经离屏的层重新点着。
  useEffect(() => {
